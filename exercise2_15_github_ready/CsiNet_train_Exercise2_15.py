...
# ────────────────────────  Exercise 2.15 Multi-Dataset Configuration  ───────── #
# Set True when you want to run Exercise 2.15 with more than five COST 2100 datasets.
use_cost2100_multi_dataset = True
# 'mixed_cost2100': concatenate all COST 2100 training/validation datasets for part (c).
# 'single_cost2100': train only on the first dataset in cost2100_datasets for ablation/debugging.
train_dataset_mode = 'mixed_cost2100'
# Set True to evaluate the trained model on every COST 2100 test dataset after training.
evaluate_all_cost2100_datasets = True
# Put the MAT files generated by COST 2100 into data/ and keep variable names HT and HF_all.
# HT shape should be [num_samples, 2048]; HF_all is optional but enables correlation calculation.
cost2100_datasets = [
    {
        'name': 'user_center',
...
# If you first run CsiNet_onlytest_Exercise2_15.py, this CSV will be used to compare part (b) and part (c).
...
# The following helper functions are added for Exercise 2.15 multi-dataset training/evaluation.
def require_file(path):
    """Check whether a required MAT file exists before loading it."""
...
def load_ht_data(path):
    """Load normalized CSI data HT from a MAT file."""
    require_file(path)
...
def load_hf_data(path):
    """Load frequency-domain CSI data HF_all from a MAT file if the file is available."""
...
def reshape_to_channels_first(x):
    """Reshape flattened CSI samples to channels_first format used by the original CsiNet code."""
...
def load_cost2100_tensor(dataset_spec, split_name):
    """Load one split from one COST 2100 dataset and reshape it for CsiNet."""
    return reshape_to_channels_first(load_ht_data(dataset_spec[split_name]))

if use_cost2100_multi_dataset:
    # Load more than five COST 2100 datasets and mix them for training when train_dataset_mode is 'mixed_cost2100'.
...
# ────────────────────────  Exercise 2.15 Evaluation Helper  ─────────────────── #
def evaluate_csi_reconstruction(model, x_eval, X_eval=None, dataset_name='dataset'):
    """Evaluate NMSE and optional correlation coefficient on one CSI dataset."""
    tStart = time.time()
...
if use_cost2100_multi_dataset and evaluate_all_cost2100_datasets:
    # Evaluate the mixed-data trained CsiNet on every COST 2100 dataset for Exercise 2.15(c).
    ex215_results = []
    x_hat = None
    rho = np.array([])
    ...
    if os.path.exists(baseline_result_csv):
        # Compare the mixed-training result in part (c) with the pretrained/single-domain result in part (b).
        baseline_rows = {}
