...
# Set True to evaluate one trained CsiNet model on more than five COST 2100 datasets for part (b).
...
# The following helper functions are added for Exercise 2.15 multi-dataset evaluation.
...
def load_cost2100_tensor(dataset_spec):
    """Load one COST 2100 test dataset and reshape it for CsiNet."""
...
if use_cost2100_multi_dataset:
    # Evaluate the pretrained CsiNet on every COST 2100 dataset for Exercise 2.15(b).
    ex215_results = []
    x_hat = None
    rho = np.array([])
...
n = min(10, len(x_test))  # Number of samples to visualize
